...
func _build_roads_and_zones() -> void:
	...
	game.apply_tool_at(T.ZONE_I_MED, ox + 16, oy + 3, ox + 19, oy + 5)
	# A farm district: low-density industrial is agricultural and may run deeper.
	game.apply_tool_at(T.ZONE_I_LOW, ox + 11, oy + 11, ox + 19, oy + 13)
...
func _process(_delta: float) -> void:
	...
	match frames:
		...
		24:
			...
			GameState.set_tool(Constants.Tool.ZONE_C_LOW)
			game.cursor_layer.begin_drag(Vector2i(town_origin.x + 1, town_origin.y + 17))
			game.cursor_layer.set_hover(Vector2i(town_origin.x + 14, town_origin.y + 24))
		28:
